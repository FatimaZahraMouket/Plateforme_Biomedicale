...
// Use the original list and apply the filter when rendering
  ...
  const handleBoitierDetail = async (boitier) => {
    ...
    const filteredBoitier = boitierpatient.payload["data"].find(item => item.patients && item.patients.id === boitier.id && item.dateFin === '0001-01-01');
    if (filteredBoitier) {
      setSelectedBoitier(filteredBoitier.boitiers);
      const filteredCapteurList = boitierCapteurList.filter(otherEntity => otherEntity.boitiers &&
  ...
  const doneestempsreel = async (patient) => {
    ...
    const filteredBoitier = boitierpatient.payload["data"].find(item =>  item.patients &&  item.patients.id === patient.id && item.dateFin === '0001-01-01');
    const filteredcapteurs = boitiercapteur.payload["data"].filter(item =>  item.boitiers && item.boitiers.id === filteredBoitier.boitiers.id);
    if (filteredBoitier) {
      setIsCodeModalOpen(true);
      setPatient(patient)
      setNombreCapteur(filteredcapteurs.length)
      setCapteurs(filteredcapteurs)
      console.log("hnaaa "+ boitiercapteur.payload["data"].filter(item =>  item.boitiers && item.boitiers.id === filteredBoitier.boitiers.id)[0].capteurs.id)
  ...
  const RecupererListPatient = async () => {
    ...
    const matchingUserExtra = extraUser.payload["data"].find((extra) =>  extra.user && extra.user.id === account.id);
    const medecinVoulu = medecinrecuperer.payload["data"].find((item) => item.extraUserId && item.extraUserId.id === matchingUserExtra?.id);
    if (medecinVoulu) {
      setListePatients(liste.payload["data"].filter((item) => item.medecins && item.medecins.id === medecinVoulu.id && item.dateFin === '0001-01-01'));
    }

...
export const BoitierUpdate = () => {
  ...
  useEffect(() => {
    if (!isNew) {
      ...
      const relatedCapteurs = boitierCapteurEntities.filter((entry) => entry.boitiers && entry.boitiers.id === boitierEntity.id);
  ...
  const saveEntity = async (values) => {
    ...
    if (isNew) {
      try {
        ...
        const relatedCapteurs = boitierCapteurEntities.filter((entry) => entry.boitiers && entry.boitiers.id === entity.id);
                    ...
                    {capteurs
                      ? capteurs.map((otherEntity) => {
                        const isLinkedToBoitier = boitierCapteurEntities.some(
                          (boitierCapteur) => boitierCapteur.capteurs && boitierCapteur.capteurs.id === otherEntity.id
                        );
                        const isAlreadyInList = elementList.some(
                          (element) => element.capteur && element.capteur.id === otherEntity.id
                        );

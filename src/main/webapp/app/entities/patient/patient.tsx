...
export const Patient = () => {
  ...
  const processedPatientList = patientList.map(patient => {
    ...
    const matchingMedecinPatient = medecinPatientEntity.find(medecinpatient =>medecinpatient && medecinpatient.patients && medecinpatient.patients.id === patient.id && medecinpatient.dateFin === '0001-01-01');
...
// Data processing logic
  const processedPatientListt = patientList.map(patient => {
    ...
    if(patient.id === selectedPatientId) {
      const matchingMedecinPatients = medecinPatientEntity.filter(medecinpatient =>  medecinpatient.patients && medecinpatient.patients.id === patient.id);
      ...
      const userExtraData = userextra.find(userExtra => patient.extraUserId && userExtra.id === patient.extraUserId.id);

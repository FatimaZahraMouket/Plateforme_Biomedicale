  ...
  const saveEntity = async values => {
    ...
    if (isNew) {
      ...
      const matchingUserExtra = userextra.find((user) => user.id === patientEntity?.extraUserId?.id);
       const userEntity = {
        id: matchingUserExtra.user ? matchingUserExtra.user.id : undefined,
        login: values.login,
        firstName: values.firstName,
        lastName: values.lastName,
        email: values.email,
                  ...
                  ? medecinList.map(otherEntity => (
                    <option value={otherEntity.id} key={otherEntity.id}>
                      ...
                      {userextra.find(
                        (userExtra) =>
                          userExtra.id &&
                          otherEntity.extraUserId &&
                          otherEntity.extraUserId.id &&
                          userExtra.id === otherEntity.extraUserId.id
                      )?.user?.lastName}{' '}
                      {userextra.find(
                        (userExtra) =>
                          otherEntity.extraUserId &&
                          otherEntity.extraUserId.id &&
                          userExtra.id &&
                          otherEntity.extraUserId.id === userExtra.id
                      )?.user?.firstName}
                    </option>
